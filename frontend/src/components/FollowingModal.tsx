...
export default function FollowingModal({ userId, open, onClose }: Props) {
  ...
  useEffect(() => {
    if (!open) return
    setLoading(true)
    usersApi.getFollowing(userId, { limit: 100 }).then((res) => {
      setItems((res.data as { items: FollowingItem[] }).items ?? [])
      setLoading(false)
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 px-4" onClick={onClose}>
      ...
      >
        <div className="flex items-center justify-between p-4 border-b border-white/10">
          <h3 className="text-lg font-bold text-white">Siguiendo</h3>
        ...
        <div className="flex-1 overflow-y-auto px-4 pb-4">
          {loading ? (
            ...
            <p className="text-gray-400 text-center py-8 text-sm">
              {search ? 'Sin resultados' : 'No sigue a nadie todavía'}
            </p>
          ) : (
            filtered.map((user) => (
